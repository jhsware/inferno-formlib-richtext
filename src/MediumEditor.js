...
import { IRichTextWidget, IRichTextEditWidget, IRichTextAction } from './interfaces'
...
class MediumEditor extends Component {
    ...
    componentDidMount () {
        ...
        editorEl.innerHTML = this._content
    ...
    didChange (e) {
        ...
        this.props.onChange(_getCleanedContent(editorEl), this._widgets)
    }
    ...
    componentWillReceiveProps (nextProps) {
        if (this.props.content !== this._content) {
            // TODO: Update content and mount/dismount widgets
        }
    }
    ...
    doAddWidget (utilityName, opt, doCloseModal) {
        ...
        const { Component } = registry.getUtility(IRichTextEditWidget, utilityName)
        
        // TODO: Add currentUser as first parameter
        // TODO: What to do with this.context.currentUser?
        const doAdd = (data, Widget, callback) => {
            ...
            // Inferno.render(<Widget allowEditing context={data} widgetId={widgetId} editor={this} onChange={this.didUpdateWidget} />, widget)
        ...
        return <Component doAdd={doAdd} onClose={doCloseModal} />
    }
    
    doDeleteWidget (widgetId) {
        var widgets = this._widgets
        var _widget = widgets[widgetId]
    ...
    render () {
        ...
        // TODO: Now we can try to use portals to mount widgets. Move this to the render method
        // Don't forget to remove unmounting, which won't be needed any more... yay!

        // IMPORTANT! Mount all widgets explicitly since they are disconnected from rest of app
        // this.widgetsLeftToMount = Object.keys(this._widgets).length
        ...
        if (this.state.editorMounted) {            
            for (var key in this._widgets) {
                ...
                try {
                    ...
                    portals.push(
                        createPortal(
                            <ViewComponent allowEditing context={widget.data} widgetId={widget.widgetId} editor={this}
                                onChange={this.didUpdateWidget}
                                onLoad={this._widgetDidLoadOrFail} />,
                            $widget
                        )
                    )
                } catch (e) {
        ...
        return (
            <div>
                <div ref={(e) => this._editorEl = e} onKeyUp={this.didChange} onClick={this.didClick} onChange={this.didChange}></div>
                {portals}
            </div>
        )
    }
